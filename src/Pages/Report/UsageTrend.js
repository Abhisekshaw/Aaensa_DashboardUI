...
const UsageTrends = (Data) => {
  ...
  console.log({ tableGraph_response });
  ...
  useEffect(() => {
    const labels = tableData.map((row, index) => `Row ${index + 1}`);
    const thermostatCutoffData = tableData.map(row => row.totalCutoffTimeOpt);
    const deviceCutoffData = tableData.map(row => row.totalCutoffTimeThrm);
        ...
        <div className="min-w-0 p-4 bg-white rounded-lg shadow-xs dark:bg-gray-800">
          <table className="w-full whitespace-wrap">
            ...
            <tbody className="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
              {/* <td className="px-4 py-3">5961488444</td>
              <td className="px-4 py-3">84661</td>
              <td className="px-4 py-3">98775152</td>
              <td className="px-4 py-3">862452</td> */}
